...
if [ "$1" == "up" ]; then
    ...
    docker compose --env-file .env -f ./deployment/compose/docker-compose.llm.yaml up
elif [ "$1" == "down" ]; then
    echo "Stopping compose for LLM"
    docker compose --env-file .env -f ./deployment/compose/docker-compose.llm.yaml down
else
